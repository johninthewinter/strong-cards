...
| `sc-firstfail-guard.sh` + `sc-firstfail-lib.sh` | `PostToolUse` / `Bash` | RULES §4.1, §14.1 | Sets a hard per-worktree lock on the first failed pytest-shaped run. Only fires on an actual `pytest`/`python -m pytest`/etc. invocation, or on pytest-summary-shaped output from a command that is not a pure read/inspection command (`sc_ff_is_readonly_inspection_command`, RULES §14.1) — this exclusion is what stops `ls`/`cat`/`tail`/`grep` on unrelated files from setting a false lock. |
| `sc-firstfail-block.sh` | `PreToolUse` / `Bash` | RULES §4.1 | **Blocks** (exit 2) any further pytest-shaped command against a worktree that already has an active first-fail lock — no blind retries. |
| `sc-clear-firstfail-lock.sh` | manual (not a Claude Code hook) | RULES §4.1 | Removes a first-fail lock, but only after recording the judge's verdict and evidence to an append-only audit log (`~/.claude/logs/sc-firstfail-audit.jsonl`). This is the only sanctioned way to clear a lock. |
| `sc-premerge-divergence-check.sh` | manual (not a Claude Code hook) | RULES §3.9 | Run by the controller before merging a card branch into an integration branch. Confirms the card branch's merge-base is the integration branch's current tip (so a merge cannot silently revert already-integrated work); `--dry-run-merge` simulates the merge in a scratch worktree and shows the resulting diff. Deliberately not wired as a blocking hook — `git merge`/`git checkout` are used for far more than card integration, and blocking them generically would be broader than the problem (RULES §8.1). |
