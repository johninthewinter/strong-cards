...
A reviewer objected that the earlier draft's "only 2 correction files exist total, at most 1 ambiguous" was misleading when read broadly, and that §1d contradicted the count by naming a different set of cases. **I re-ran the enumeration rather than accept either the original claim or the objection.** Sweep: every `*-corrected.json`, `*-original.json` and `metadata-correction.json` under every `cards/*/reviews/` path in `/Users/misterj/src/The_Builder`, worktree copies excluded as byte-identical duplicates of the same tracked files (`strong-cards_wip` has no `cards/` directory at all). Result — **exactly 4 correction *cases* across 24 review directories** (re-counted after a v2 review flagged the earlier draft's "25" as one over the true directory count; re-verified directly), drawn from **6 literal files, not 4** — case 3 spans an original/current pair (`review-original.json` + `review.json`) and case 4 spans a superseded/superseding pair (`review.json` + `review-corrected.json`); cases 1 and 2 are each one `metadata-correction.json` file. "Four" below means four distinct correction *events*, never a literal file count. Here is all four, in full, with no rounding and no hedging:
...
*Containment is in scope, not deferred (v2 correction, §1a).* Typed arguments reduce injection; they do **not** remove the trust boundary. Every probe runs contained — `--network none`, `--cap-drop=ALL`, `--read-only`, per-probe timeout, output byte cap — and every path argument is containment-checked **after** `realpath` resolution against the repo root, closing symlink escape. *`realpath` alone does not close every TOCTOU window* (v2 correction) — a check-then-open race remains possible if the resolved path's target changes between the resolve and the actual open; SC-05's implementation must open with `O_NOFOLLOW`/`openat`-style resolve-then-hold semantics (resolve once, keep the descriptor, never re-resolve the string) rather than treat a single `realpath` call as sufficient on its own. Regex arguments carry a match budget. The `sc/receipts-gate` container hardening is the starting point for this, which is why the branch is archived rather than deleted. Dropping containment is **not** authorized by this plan and may only be revisited after SC-05 ships and its argument handling is independently audited. Includes real forward *and* reverse reachability — reverse actually runs a search rather than checking that a `B<n>` line exists. Archive `sc/receipts-gate` under a tag; do not merge. Rewrite RULES §27 to describe the shipped mechanism.
...
Controller stamps the dispatch envelope (requested model, effort, host acknowledgment, card + source hashes, worktree, run ID, timestamps); the envelope is authoritative and a review whose self-reported model disagrees is **refused, not corrected**. Verdicts are `tee`'d mechanically to `docs/reviews/<card-id>/<round>-<model>.json` as part of the dispatch's own call — zero extra model cost. **Scope explicitly includes planning dispatches, not only card reviews** (v2 addition, repeated here rather than left only in §1b/§1c's intro): the reason several of this plan's own disputed claims about a prior planning dispatch's exact content were unverifiable is that planning output is not persisted anywhere on disk today — the same gap this card exists to close for reviews. `tee` planning-dispatch output to `docs/reviews/<plan-id>/<round>-<model>-plan.json` on the same code path.
...
- the comparison also covers **mtime/inode** as a backstop signal, *not a guarantee* (v2 correction) — an in-place write that restores identical bytes can also restore identical mtime/inode on some filesystems/tools, so this check catches the common case but must not be the only control; the read-only mount above is what actually prevents the write, and a controller-visible write-attempt log (e.g. the mount reporting any attempted write syscall, not just its outcome) is required wherever the mount's read-only enforcement itself is in question;
