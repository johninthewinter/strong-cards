...
**What happened.** Composer LC-02b-R1's ledger claimed one browser gate could show lesson, output preview, a live wait tag, X-Ray, and the strip together. In the live harness, those states came from incompatible or separate routes: output preview is an ended run, the wait tag exists only during a running event, and X-Ray/lesson have independent setup. The card named “an existing client-side seam” but no file, export, deterministic setup sequence, or state-retention contract. It also omitted active X-Ray bar/readout rows from the allocator it claimed was shared.

**How to apply.** A composite UI gate must list every visible state in a reachability table: state producer, exact tracked fixture/action or test-only seed seam, owning file, whether it can coexist with every other state, and the assertion proving it. “Use an existing seam,” DOM injection, or untracked state mutation is invalid unless the exact exported seam and its setup are named in the Touch List. If production event ordering makes requested states mutually exclusive, the card must add a bounded explicit test seed/state-retention seam or test reachable alternatives; it may not claim the composite proof. A shared allocator's consumer census includes every currently active row in the contested region—not only the feature names already mentioned in the card—and the browser fixture must activate each included row at least once.
...
**What happened.** Composer SRA-01A was correctly narrowed to one additive allocator module, but its draft named ordering and duplicate behavior without specifying the key union, priority table, slot props, invalid-child policy, or the focused render harness. Its unchanged output-preview test proved behavior, not the claimed pixel identity. A small scope is still not implementable when callers and tests cannot observe its exact contract.

**How to apply.** An inert foundation card must declare every exported type, constant, component prop, closed vocabulary, priority order, unknown/duplicate behavior, and invalid-child/fragment/multiple-child policy that a successor will rely on. Its ledger names the exact render or browser harness that loads the new module and separates markup assertions from geometry assertions. Claims of unchanged pixels require a visual or geometry proof; otherwise state the narrower behavioral/static no-consumer proof. Before freeze, supersede any broad predecessor and name the serial successor map so no later card depends on an ambiguous generic label.

**Exactness addendum.** Public-API proof must cover both runtime and type-only exports, and must state the development/test versus production load mode it actually exercises. An inertness claim needs an audit for stylesheet imports and global/import-time mutation, not merely zero callers. Hygiene gates distinguish unignored worktree files from ignored dependency/cache paths and name any allowed ignored paths. A serial successor map is incomplete until every existing tracker reference has been swept; no generic predecessor label may remain in a dependent feature entry.
